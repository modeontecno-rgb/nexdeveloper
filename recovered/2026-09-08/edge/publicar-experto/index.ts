// Edge Function «publicar-experto»: sube expertos/<slug>/SKILL.md al repositorio privado modeontecno-rgb/expertos-red.
...
type Experto = { id: string; user_id: string; slug: string; nombre: string; papel: string; cuando_usarlo: string | null; instrucciones: string | null; url_origen: string | null };
...
function contenido(e: Experto) {
  return [`# ${e.nombre}`, "", `**Papel:** ${e.papel}`, `**Cuándo usarlo:** ${e.cuando_usarlo ?? "sin indicar"}`, `**Origen:** ${e.url_origen ?? "sin indicar"}`, "", "---", "", e.instrucciones ?? "_Sin instrucciones todavía._", ""].join("\n");
}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // El experto debe ser del usuario que llama.
    const comoUsuario = createClient(url, Deno.env.get("SUPABASE_ANON_KEY")!, { global: { headers: { Authorization: req.headers.get("Authorization") ?? "" } } });
    const { data: sesion } = await comoUsuario.auth.getUser();
    if (!sesion?.user) return responder({ ok: false, error: "Sin sesión." }, 401);

    const { data: experto, error } = await servicio.from("expertos").select("id, user_id, slug, nombre, papel, cuando_usarlo, instrucciones, url_origen").eq("id", expertoId).eq("user_id", sesion.user.id).maybeSingle();
    if (error || !experto) return responder({ ok: false, error: "No se encuentra el experto." }, 404);

    const token = Deno.env.get("GITHUB_TOKEN");
    if (!token) {
      const { data: proyecto } = await servicio.from("proyectos").select("id").eq("user_id", experto.user_id).limit(1).maybeSingle();
      if (proyecto) {
        await servicio.from("tareas").insert({
          user_id: experto.user_id, proyecto_id: proyecto.id,
          titulo: "Publicar experto en GitHub", descripcion: `Falta el token de GitHub para publicar «${experto.nombre}».`,
          requiere_atencion: true, motivo_atencion: "Falta GITHUB_TOKEN",
          instrucciones: "Añade el secreto GITHUB_TOKEN a las Edge Functions de Supabase y vuelve a adoptar el experto.",
        });
      }
    ...
    const subida = await fetch(api, {
      ...
      body: JSON.stringify({ message: `Experto ${experto.slug}`, content: btoa(unescape(encodeURIComponent(contenido(experto as Experto)))), ...(sha ? { sha } : {}) }),
    });
