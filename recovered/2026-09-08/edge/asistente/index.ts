...
async function proveedoresDisponibles(sb: SB, userId: string): Promise<Prov[]> {
  const { data: provs } = await sb.from("proveedores_ia").select("id, clave_slug, activo, clave_cifrada").eq("user_id", userId).eq("activo", true).not("clave_cifrada", "is", null).in("clave_slug", ["abacus", "anthropic", "openai", "groq", "mistral", "deepseek", "xai", "openrouter"]);
  const out: Prov[] = [];
