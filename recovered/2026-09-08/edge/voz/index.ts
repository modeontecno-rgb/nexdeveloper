...
// Acciones: voces · generar_guion {proyecto_id, duracion_objetivo_seg?, publico?, notas?} · locutar {guion_id?, escena?, texto?, titulo?, proyecto_id?, voz_id?} · enlace {locucion_id, descargar?} · sin acción → estado
// 0.36.0: redactar() prueba primero Abacus (RouteLLM), incluido sin coste extra en la suscripción.
...
async function redactar(sb: SB, userId: string, sistema: string, pregunta: string) {
  for (const slug of ["abacus", "anthropic", "groq", "google"]) {
    ...
    try {
      ...
      else if (slug === "abacus") { const r = await fetch("https://routellm.abacus.ai/v1/chat/completions", { method: "POST", headers: { Authorization: `Bearer ${k}`, "content-type": "application/json" }, body: JSON.stringify({ model: "route-llm", temperature: 0.4, response_format: { type: "json_object" }, messages: [{ role: "system", content: sistema }, { role: "user", content: pregunta }] }) }); if (r.ok) { const j = await r.json(); return { texto: j.choices?.[0]?.message?.content ?? "", por: slug }; } }
