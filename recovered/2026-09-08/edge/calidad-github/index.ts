// Edge Function «calidad-github» (verify_jwt=false: la autenticación se hace dentro).
// Modo usuario: cabecera Authorization con la sesión. Modo programado: cabecera
// x-cron-token que coincide con private.claves_sistema.cron_token (RPC comprobar_cron_token).
...
type FilaResultado = { codigo: string; resultado: Resultado; detalle?: string; metrica?: Record<string, unknown>; url_detalle?: string };
...
function normalizar(repositorio: string) {
  return repositorio.trim().replace(/^https?:\/\/(www\.)?github\.com\//, "").replace(/\.git$/, "").replace(/\/+$/, "");
}

const CONTROLES_BLOQUEANTES = ["tipos", "compilacion", "vitest", "advisors_seguridad", "secretos_codigo", "reglas_fabricante"];
...
async function avisosSupabase(refProyecto: string | null, token: string | null): Promise<FilaResultado[]> {
  ...
  const consultar = async (tipo: "security" | "performance"): Promise<FilaResultado> => {
    ...
    try {
      const respuesta = await fetch(`https://api.supabase.com/v1/projects/${refProyecto}/advisors/${tipo}`, { headers: { Authorization: `Bearer ${token}` } });
      if (!respuesta.ok) return { codigo, resultado: "omitido", detalle: `La API de Supabase ha respondido ${respuesta.status}` };
      ...
      if (errores > 0) return { codigo, resultado: "fallo", detalle: `${errores} avisos de nivel error`, metrica };
...
Deno.serve(async (req: Request) => {
  ...
  try {
    const cuerpo = (await req.json().catch(() => ({}))) as { accion?: string; proyecto_id?: string; version?: string };
    const accion = cuerpo.accion ?? "sincronizar";
    if (accion === "sugerir_taller") return responder({ ok: true, taller: TALLER });
    ...
    if (cronToken) {
      ...
      if (!datosUsuario?.user) return responder({ ok: false, error: "Debes haber iniciado sesión." }, 401);
    ...
    if (accion === "lanzar") {
      if (programado) return responder({ ok: false, error: "Lanzar requiere sesión de usuario." }, 403);
      if (!cuerpo.proyecto_id) return responder({ ok: false, error: "Falta el proyecto." }, 400);
      const { data: proyecto } = await servicio.from("proyectos").select("id, user_id, nombre, repositorio").eq("id", cuerpo.proyecto_id).maybeSingle();
      if (!proyecto) return responder({ ok: false, error: "No se encuentra el proyecto." }, 404);
      if (proyecto.user_id !== usuarioId) return responder({ ok: false, error: "El proyecto no es tuyo." }, 403);
      if (!proyecto.repositorio) return responder({ ok: false, error: "El proyecto no tiene repositorio indicado." }, 400);
      ...
      const lanzada = await fetch(`https://api.github.com/repos/${repo}/actions/workflows/${TALLER}/dispatches`, {
        method: "POST", headers: cabeceras(token), body: JSON.stringify({ ref: rama, inputs: { version } }),
      });
      if (lanzada.status === 404) {
        await servicio.from("tareas").insert({
          user_id: proyecto.user_id, proyecto_id: proyecto.id,
          ...
          requiere_atencion: true, motivo_atencion: "Falta el taller de calidad",
          instrucciones: "Copia el fichero calidad.yml que aparece en la tarjeta de ayuda de la pantalla Calidad a la carpeta .github/workflows/ del repositorio.",
        });
      ...
      const { data: ejecucion, error } = await servicio.from("ejecuciones_calidad")
        .insert({ user_id: proyecto.user_id, proyecto_id: proyecto.id, version, origen: "github_actions", estado: "en_cola" })
        .select("id").maybeSingle();
      if (error) return responder({ ok: false, error: error.message }, 500);
      return responder({ ok: true, ejecucion_id: ejecucion?.id ?? null });
    }

    let consulta = servicio.from("ejecuciones_calidad").select("id, user_id, proyecto_id, version, iniciada_el, run_id_github").in("estado", ["en_cola", "ejecutando"]);
    if (!programado && usuarioId) consulta = consulta.eq("user_id", usuarioId);
    const { data: abiertas } = await consulta;
    ...
    for (const ejecucion of abiertas) {
      const { data: proyecto } = await servicio.from("proyectos").select("id, user_id, nombre, repositorio, supabase_ref").eq("id", ejecucion.proyecto_id).maybeSingle();
      if (!proyecto?.repositorio) continue;
      const repo = normalizar(proyecto.repositorio);
      const listado = await fetch(`https://api.github.com/repos/${repo}/actions/runs?event=workflow_dispatch&per_page=10`, { headers: cabeceras(token) });
      if (!listado.ok) continue;
      const runs = ((await listado.json()) as { workflow_runs?: { id: number; html_url: string; status: string; conclusion: string | null; created_at: string; path?: string }[] }).workflow_runs ?? [];
      const inicio = new Date(ejecucion.iniciada_el as string).getTime() - 120_000;
      const run = runs.find((r) => r.id === Number(ejecucion.run_id_github)) ??
        runs.filter((r) => (r.path ?? "").includes(TALLER) && new Date(r.created_at).getTime() >= inicio)
          .sort((a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime())[0];
      if (!run) continue;
      if (run.status !== "completed") {
        await servicio.from("ejecuciones_calidad").update({ estado: "ejecutando", run_id_github: run.id, url_run: run.html_url }).eq("id", ejecucion.id);
        continue;
      }
      let filas: FilaResultado[] = [];
      const artefactos = await fetch(`https://api.github.com/repos/${repo}/actions/runs/${run.id}/artifacts`, { headers: cabeceras(token) });
      if (artefactos.ok) {
        ...
        if (artefacto) {
          const zip = await fetch(`https://api.github.com/repos/${repo}/actions/artifacts/${artefacto.id}/zip`, { headers: cabeceras(token) });
          if (zip.ok) { try { filas = await leerZip(new Uint8Array(await zip.arrayBuffer())); } catch { filas = []; } }
        }
      }
      const refSupabase = (proyecto as { supabase_ref?: string | null }).supabase_ref ?? null;
      ...
      if (filas.length > 0) {
        await servicio.from("resultados_calidad").insert(filas.map((f) => ({
          ejecucion_id: ejecucion.id, control_codigo: f.codigo, resultado: f.resultado, detalle: f.detalle ?? "", metrica: f.metrica ?? {}, url_detalle: f.url_detalle ?? run.html_url,
        })));
      }
      const estado = run.conclusion === "cancelled" ? "error" : calcularEstado(filas);
      const resumen = {
        ok: filas.filter((f) => f.resultado === "ok").length, aviso: filas.filter((f) => f.resultado === "aviso").length,
        fallo: filas.filter((f) => f.resultado === "fallo").length, omitido: filas.filter((f) => f.resultado === "omitido").length,
      };
      const terminada = new Date();
      await servicio.from("ejecuciones_calidad").update({
        estado, run_id_github: run.id, url_run: run.html_url, terminada_el: terminada.toISOString(),
        duracion_seg: Math.round((terminada.getTime() - new Date(ejecucion.iniciada_el as string).getTime()) / 1000), resumen,
      }).eq("id", ejecucion.id);
      cerradas += 1;
      if (estado === "rojo" || estado === "error") {
        const fallidos = filas.filter((f) => f.resultado === "fallo").map((f) => f.codigo).join(", ");
        await servicio.from("tareas").insert({
          user_id: ejecucion.user_id, proyecto_id: ejecucion.proyecto_id,
          ...
          requiere_atencion: true, motivo_atencion: "Control de calidad en rojo",
