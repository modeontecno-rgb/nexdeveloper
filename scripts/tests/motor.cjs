...
function load(o={}){
 ...
 const db={from(table){let columns='';const q={select(s){columns=s;return q},eq(){return q},gt(){return q},neq(){return q},update(){return q},maybeSingle:async()=>({data:table==='proveedores_ia'?{id:'provider'}:table==='modelos_ia'?{id:'model'}:{id:e.id},error:null}),single:async()=>({data:table==='ejecuciones_orden'?{...e,estado:o.cancelled?'cancelada':e.estado}:null,error:null})};return q},async rpc(name,args){rpcCalls.push({name,args});return {data:name==='gasto_ia_permitido'?true:name==='descifrar_clave_proveedor'?'fixture-key':name==='reclamar_ejecucion'?!o.busy:true,error:null}}};
 return {f:ctx.engineTests,db,e,calls,providerCalls,rpcCalls};
}
