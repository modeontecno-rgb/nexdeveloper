...
async function scenario(o={}){
 let sent=0,rpcs=[]; const ctx={exports:{},Response,URL,TextEncoder,AbortSignal,crypto:require('node:crypto').webcrypto,fetch:async()=>{sent++;if(o.timeout)throw Object.assign(Error('timeout'),{name:'TimeoutError'});if(o.transportError)throw Error('Network lost');return new Response(JSON.stringify(o.reply??{usage:{prompt_tokens:100,completion_tokens:50},choices:[{message:{content:'Fixture'}}]}),{status:o.status??200})}};
 ...
 let body,error,details;try{const response=await ctx.exports.fetchIA(db,{userId:'user',modeloId:'model',ambito:'personal',operacion:'test'},o.url??'https://api.deepseek.com/v1/chat/completions',{method:'POST',body:JSON.stringify({model:'fixture-model',max_tokens:100,...o.body})});body=await response.json()}catch(e){error=e.message;details={codigo:e.codigo,coste:e.coste,entrada:e.entrada,salida:e.salida}}return {sent,rpcs,body,error,details};
}
(async()=>{let tests=[];async function t(name,o,p){let r=await scenario(o);tests.push({name,passed:!!p(r)});}
 ...
 await t('F13 Truncated output is never presented as complete',{reply:{usage:{prompt_tokens:100,completion_tokens:100},choices:[{finish_reason:'length',message:{content:'cut off'}}]}},r=>r.sent===1&&r.error&&!r.body&&r.rpcs.some(x=>x.args.p_salida===100)&&r.details.codigo==='SALIDA_TRUNCADA_CONTABILIZADA'&&r.details.coste===.001&&r.details.salida===100);
 ...
 await t('F18 Timeout explained without a retry or refund',{timeout:true},r=>r.sent===1&&r.error.includes('60 segundos')&&r.rpcs[1].args.p_entrada===null);
