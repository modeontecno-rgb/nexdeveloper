-- Retire arbitrary service-role SQL. p_user was previously unused.
CREATE OR REPLACE FUNCTION public.asistente_consulta(p_user uuid,p_sql text) RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path=public,pg_temp AS $$
BEGIN RAISE EXCEPTION 'Consulta SQL libre retirada. Usa la consulta estructurada del asistente'; END $$;
REVOKE ALL ON FUNCTION public.asistente_consulta(uuid,text) FROM PUBLIC,anon,authenticated,service_role;
