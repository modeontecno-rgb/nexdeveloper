import {comprobarTextoProtegido} from '../_shared/estilo.ts';
...
// (Abacus, Anthropic, Google, OpenAI, Groq, Mistral, DeepSeek, xAI, OpenRouter) y un «juez» fusiona las respuestas en una sola,
// señalando discrepancias. Documentos personales en la carpeta PERSONAL del almacén (nunca en carpetas de proyectos)
// y nunca en Proyectian. Anota el consumo en consumos_ia sin proyecto. Editor de estilo propio (tu voz, guía de marca, tutor).
...
async function llamar(sb: SB, userId: string, p: Prov, sistema: string, turnos: Turno[], maxTokens = 2500, proyectoId: string | null = null) {
  if (p.slug === "anthropic") {
    const r = await fetchIA(sb, { userId, modeloId: p.modelo_id!, ambito: proyectoId ? "proyecto" : "personal", proyectoId, operacion: "personal" }, "https://api.anthropic.com/v1/messages", { method: "POST", headers: { "x-api-key": p.clave, "anthropic-version": "2023-06-01", "content-type": "application/json" }, body: JSON.stringify({ model: p.modelo, max_tokens: maxTokens, system: sistema, messages: turnos }) });
    if (!r.ok) throw new Error(`Anthropic ${r.status}: ${(await r.text()).slice(0, 160)}`); const j = await r.json();
    return { coste: j._nex_coste_eur as number, texto: (j.content ?? []).map((c: any) => c.text ?? "").join(""), te: j.usage?.input_tokens ?? 0, ts: j.usage?.output_tokens ?? 0 };
  }
  if (p.slug === "google") {
    const contents = turnos.map((t) => ({ role: t.role === "assistant" ? "model" : "user", parts: [{ text: t.content }] }));
    const r = await fetchIA(sb, { userId, modeloId: p.modelo_id!, ambito: proyectoId ? "proyecto" : "personal", proyectoId, operacion: "personal" }, `https://generativelanguage.googleapis.com/v1beta/models/${p.modelo}:generateContent?key=${p.clave}`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ systemInstruction: { parts: [{ text: sistema }] }, contents, generationConfig: { maxOutputTokens: maxTokens, temperature: 0.4 } }) });
    if (!r.ok) throw new Error(`Google ${r.status}: ${(await r.text()).slice(0, 160)}`); const j = await r.json();
    return { coste: j._nex_coste_eur as number, texto: (j.candidates?.[0]?.content?.parts ?? []).map((x: any) => x.text ?? "").join(""), te: j.usageMetadata?.promptTokenCount ?? 0, ts: j.usageMetadata?.candidatesTokenCount ?? 0 };
  }
  ...
  const r = await fetchIA(sb, { userId, modeloId: p.modelo_id!, ambito: proyectoId ? "proyecto" : "personal", proyectoId, operacion: "personal" }, `${bases[p.slug]}/chat/completions`, { method: "POST", headers: { Authorization: `Bearer ${p.clave}`, "content-type": "application/json" }, body: JSON.stringify({ model: p.modelo, max_tokens: maxTokens, temperature: 0.4, messages: [{ role: "system", content: sistema }, ...turnos] }) });
  if (!r.ok) throw new Error(`${p.slug} ${r.status}: ${(await r.text()).slice(0, 160)}`); const j = await r.json();
  return { coste: j._nex_coste_eur as number, texto: j.choices?.[0]?.message?.content ?? "", te: j.usage?.prompt_tokens ?? 0, ts: j.usage?.completion_tokens ?? 0 };
}
...
async function responder(sb: SB, userId: string, cfg: any, historial: Turno[], pregunta: string) {
  ...
  if (!provs.length) throw new Error("No hay ningún proveedor de IA de texto con clave (Abacus, Anthropic, Google, OpenAI, Groq…). Ponla en Ajustes → Proveedores.");
  if (Array.isArray(cfg.proveedores) && cfg.proveedores.length) provs = provs.filter((p) => cfg.proveedores.includes(p.slug));
  if (!provs.length) throw new Error("Los proveedores seleccionados no están disponibles; revisa la configuración.");
  const juez = provs.find((p) => p.slug === cfg.juez) ?? provs[0];
  const modo = cfg.modo ?? "rapido";
  const participantes = modo === "rapido" ? [juez] : provs.slice(0, Number(cfg.max_proveedores ?? 1));
  ...
  const resultados = await Promise.all(participantes.map(async (p) => {
    const ini = Date.now();
    try { const r = await conTiempo(llamar(sb, userId, p, sistema, turnos, 2200)); return { proveedor: p.slug, nombre: p.nombre, modelo: p.modelo, texto: r.texto, tokens_entrada: r.te, tokens_salida: r.ts, coste: r.coste, ms: Date.now() - ini, error: null as string | null, p }; }
  ...
  if (modo === "fusion" && validas.length > 1) {
    const sistemaJ = `${SISTEMA_BASE(cfg.instrucciones)}\n\nAhora actúas como JUEZ: has recibido la misma pregunta respondida por varias IA. Escribe UNA sola respuesta final para Javier, la mejor posible, tomando lo más correcto, completo y útil de cada una, corrigiendo errores evidentes y sin decir «segun la IA X». Al final, si las respuestas discrepan en algo relevante, añade una sección corta «Dónde no coinciden» (2-4 frases); si coinciden, no la añadas.`;
    const preg = `Pregunta de Javier:\n${pregunta}\n\n${validas.map((r, i) => `=== Respuesta ${i + 1} (${r.nombre} · ${r.modelo}) ===\n${r.texto}`).join("\n\n")}`;
    try {
      const rj = await conTiempo(llamar(sb, userId, juez, sistemaJ, [...historial.slice(-6), { role: "user", content: preg }], 3000), 70_000);
      texto = rj.texto; teJ = rj.te; tsJ = rj.ts; costeJ = rj.coste; juezUsado = juez.slug;
...
async function titular(_sb: SB, _userId: string, pregunta: string) {
  return pregunta.trim().replace(/\s+/g, " ").slice(0, 80) || "Nueva conversación";
}
  ...
  try {
    ...
    switch (accion) {
      ...
      case "preguntar": {
        ...
        const cfg = await config(sb, userId);
        const { data: permitido, error: errorPresupuesto } = await sb.rpc("gasto_ia_permitido", { p_user_id: userId, p_proveedor: null, p_proyecto_id: null });
        if (errorPresupuesto || permitido !== true) return json({ ok: false, error: "Presupuesto de IA superado con acción «bloquear». Revisa Gasto de IA → Presupuestos." }, 400);
        ...
        const { data: previos, error: historialError } = await sb.from("personal_mensajes").select("rol, texto").eq("user_id",userId).eq("conversacion_id", convId).order("fecha", {ascending:false}).order("id",{ascending:false}).limit(30);
        if(historialError) throw new Error("No se ha podido recuperar el historial. No se ha enviado la pregunta a la IA.");
        const historial: Turno[] = (previos ?? []).reverse().map((m) => ({ role: m.rol === "usuario" ? "user" : "assistant", content: m.texto }));
        const { data: mu, error: preguntaError } = await sb.from("personal_mensajes").insert({ user_id: userId, conversacion_id: convId, rol: "usuario", texto: pregunta, adjuntos: cuerpo.adjuntos ?? [] }).select("*").single();
        if(preguntaError || !mu) throw new Error("No se ha guardado la pregunta; no se ha llamado a la IA.");
        const r = await responder(sb, userId, cfg, historial, pregunta);
        const { data: ma, error: respuestaError } = await sb.from("personal_mensajes").insert({ user_id: userId, conversacion_id: convId, rol: "asistente", texto: r.texto, respuestas: r.respuestas, juez: r.juez, discrepancias: r.discrepancias, tokens_entrada: r.te, tokens_salida: r.ts, coste: r.coste }).select("*").single();
        if(respuestaError || !ma) return json({ok:false,error:"La IA respondió, pero no se pudo guardar. No reenvíes automáticamente la pregunta.",respuesta_recuperable:r.texto,conversacion_id:convId},500);
      ...
      case "crear_documento": {
        ...
        if (!md && cuerpo.conversacion_id) {
          ...
          const r = await conTiempo(llamar(sb, userId, p, `${SISTEMA_BASE(cfg.instrucciones)}\n\nRedacta un ${tipo} en Markdown, completo y bien estructurado (título con #, secciones con ##), a partir de la conversación. Sin preambulos.`, [{ role: "user", content: (msgs ?? []).map((m) => `${m.rol === "usuario" ? "Javier" : "Asistente"}: ${m.texto}`).join("\n\n").slice(0, 60000) + (cuerpo.indicaciones ? `\n\nIndicaciones: ${cuerpo.indicaciones}` : "") }], 4000), 90_000);
      ...
      case "reescribir": {
        ...
        const modo = ["desactivado","natural","mi_voz", "marca", "tutor"].includes(cuerpo.modo) ? cuerpo.modo : "desactivado";
        if(texto.length>40000)return json({ok:false,error:'Divide el texto en secciones de hasta 40.000 caracteres; no se recorta automáticamente.'},400);
        if(modo==='desactivado')return json({ok:true,modo,texto_original:texto,texto_resultado:texto,coste:0,notas:null});
        if(texto.trim().length<20)return json({ok:false,error:'Introduce al menos 20 caracteres para revisar el estilo.'},400);
        const cfg = await config(sb, userId);
        const provs = await proveedoresDisponibles(sb, userId); const p = provs[0]; if (!p) return json({ ok: false, error: "Sin proveedor de IA de texto con clave" }, 400);
        let guia: string | null = null; let proyectoId: string | null = null;
        if (cuerpo.proyecto_id) { const { data: pr } = await sb.from("proyectos").select("id, nombre, guia_estilo, descripcion").eq("id", String(cuerpo.proyecto_id)).eq("user_id", userId).maybeSingle(); if (pr) { proyectoId = pr.id; guia = pr.guia_estilo ?? `Tono cercano y claro para los usuarios de ${pr.nombre} (${pr.descripcion ?? ""}).`; } }
        if((modo==='marca'||cuerpo.proyecto_id)&&!proyectoId)return json({ok:false,error:'Selecciona un proyecto autorizado para su guía de estilo.'},400);
        if(modo==='mi_voz'&&!cfg.perfil_estilo)return json({ok:false,error:'Guarda tu perfil de estilo o selecciona Natural.'},400);
        ...
        if (modo === "tutor") {
          ...
          const perfil = modo === "natural" ? "Escribe con naturalidad, claridad y precisión, sin imitar a una persona concreta." : modo === "marca" ? `GUÍA DE ESTILO DEL PROYECTO:\n${guia}` : `PERFIL DE ESTILO DE JAVIER:\n${cfg.perfil_estilo ?? "(sin perfil aún: escribe claro, directo, cercano, frases cortas, español de España)"}`;
          sistema = `Eres el editor personal de Javier. Reescribes el texto que te dan para que suene como lo escribiría él (o como pide la guía), manteniendo TODO el contenido, los datos y la intención; conserva literalmente cifras, URLs y bloques de código; quitas relleno, repeticiones y frases hechas; no inventas nada. ${perfil}\n${cuerpo.tono ? `Tono pedido: ${cuerpo.tono}.` : ""}\nDevuelve SOLO el texto reescrito, sin comentarios.`;
        }
        const r = await conTiempo(llamar(sb, userId, p, sistema, [{ role: "user", content: texto.slice(0, 40000) }], 4000, proyectoId), 90_000);
        const c = r.coste;

        if (modo === "tutor") { let j: any = null; try { j = JSON.parse((r.texto.match(/\{[\s\S]*\}/) ?? [r.texto])[0]); } catch { j = { valoracion: r.texto }; } salida = { notas: j }; }
        else {comprobarTextoProtegido(texto,r.texto.trim());salida = { texto_resultado: r.texto.trim(), texto_original:texto };}
        const { data: reg, error: escrituraError } = await sb.from("estilo_reescrituras").insert({ user_id: userId, proyecto_id: proyectoId, modo, texto_original: texto, texto_resultado: salida.texto_resultado ?? null, notas: salida.notas ?? null, coste: c }).select("id").single();
        if(escrituraError||!reg)return json({ok:false,error:'No se pudo guardar la revisión. Conserva esta respuesta antes de reintentar.',respuesta_recuperable:salida},500);
      ...
      case "guia_estilo_proyecto": {
        ...
        if (!guia) { const provs = await proveedoresDisponibles(sb, userId); const p = provs[0]; if (!p) return json({ ok: false, error: "Sin proveedor de IA" }, 400); const r = await conTiempo(llamar(sb, userId, p, "Redactas guías de estilo breves (10-15 líneas) para los textos que ve el cliente de una aplicación: tono, tratamiento (tú/usted), vocabulario, lo que evitar, ejemplos de frases. Español de España. Sin preambulos.", [{ role: "user", content: `Aplicación: ${pr.nombre}. ${pr.descripcion ?? ""} Objetivo: ${pr.objetivo ?? ""}. ${cuerpo.indicaciones ?? ""}` }], 900, pr.id), 60_000); guia = r.texto.trim();  }
