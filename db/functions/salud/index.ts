...
const URL_FUNCION = `${URL_SUPABASE}/functions/v1/salud`;
...
async function incidenciasSentry(org: string, proyectoSlug: string) {
  // Busca el proyecto por slug o nombre; devuelve incidencias sin resolver de las últimas 24 h
...
async function comprobarProyecto(sb: SB, cfg: any, fila: any, estadosCuenta: Record<string, any>, orgSentrySlug: string | null) {
  ...
  try {
    ...
    // 1) Estado del proyecto en la cuenta
    ...
    if (semaforo === "rojo" && (est?.status === "INACTIVE" || est?.status === "PAUSED")) { datos.semaforo = "rojo"; datos.motivos = motivos; await sb.from("salud_proyectos").update(datos).eq("id", fila.id); return "rojo"; }
    // 2) Servicios
    ...
    // 4) SQL: RLS, search_path, tamaño, usuarios
    try {
    ...
    // 5) Errores en 24 h (logs unificados, ClickHouse)
    try {
...
async function continuar(sb: SB, informeId: string) {
  ...
  try { const lista = await admin("/projects"); for (const p of lista ?? []) estados[p.id] = p; } catch (e) { estados = {}; }
...
async function cerrarInforme(sb: SB, inf: any, cfg: any) {
  ...
  for (const f of avisar) {
    // Una sola tarea abierta por proyecto y motivo principal
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    switch (accion) {
      ...
      case "comprobar": {
        // Todos los proyectos o uno solo; responde cuando termina la primera tanda y continúa sola
