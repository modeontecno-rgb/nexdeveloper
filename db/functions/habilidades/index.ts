...
// ---------- Sincronizar desde el repositorio de propias (carpetas habilidades/ y expertos/) ----------
...
// ---------- Barrido de habilidades externas en GitHub ----------
async function barrer(sb: SB, userId: string, cfg: any) {
  ...
  for (const tema of (cfg.temas_barrido ?? []).slice(0, 8)) {
    ...
    if (!r.ok) { if (r.status === 403 || r.status === 422) continue; continue; }
  ...
  // Índice en el repositorio de externas
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    switch (accion) {
      ...
      case "guardar": {
        // Crea o edita una habilidad propia: BD + SKILL.md en el repositorio de propias
      ...
      case "invocar": {
        // Crea una tarea (o una orden) en el proyecto con las instrucciones de la habilidad
        const { data: h } = await sb.from("habilidades").select("*").eq("id", String(cuerpo.habilidad_id)).eq("user_id", userId!).maybeSingle();
        if (!h) return json({ ok: false, error: "No encontrada" }, 404);
